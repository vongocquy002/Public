  ...
  while (1) {
    ...
    if (isConnected) {
      ...
      if (clients[clientId].available() >= 7) {
        uint8_t buffer[32]; // Tăng buffer để xử lý dữ liệu gộp
        ...
        // Di chuyển buffer đến frame hợp lệ
        int frameLen = buffer[frameStart + 4] + 7; // dataLen + 7
        ...
        if (frameLen >= 7 && frameLen <= len && (carId <= CAR5 || carId == ALL_CARS)) {
          ...
          if (checksum == buffer[frameStart + frameLen - 2]) {
            ...
            switch (cmd) {
              case STATUS_REPORT:
                if (dataLen == 9) {
                  bool isOn = buffer[frameStart + 5];
                  uint8_t speed = buffer[frameStart + 6];
                  uint8_t mode = buffer[frameStart + 7];
                  uint8_t gpio[6];
                  memcpy(gpio, buffer + frameStart + 8, 6);
            ...
            if (xSemaphoreTake(clientMutex, pdMS_TO_TICKS(1500)) == pdTRUE) {
              ...
              xSemaphoreGive(clientMutex);
              vTaskDelay(50 / portTICK_PERIOD_MS); // Thêm delay để client xử lý "OK"
            }
          } else {
