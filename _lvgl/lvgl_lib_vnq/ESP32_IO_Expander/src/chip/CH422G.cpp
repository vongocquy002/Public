...
static esp_err_t write_od_output_reg(esp_io_expander_handle_t handle, uint32_t value);
static esp_err_t read_output_reg(esp_io_expander_handle_t handle, uint32_t *value);
static esp_err_t read_od_output_reg(esp_io_expander_handle_t handle, uint32_t *value);
...
static esp_err_t esp_io_expander_new_i2c_ch422g(i2c_port_t i2c_num, uint32_t i2c_address, esp_io_expander_handle_t *handle)
{
    ...
    ch422g->regs.od_output = OD_OUT_REG_DEFAULT_VAL;
    ch422g->base.read_input_reg = read_input_reg;
    ch422g->base.write_output_reg = write_output_reg;
    ch422g->base.write_od_output_reg = write_od_output_reg;
    ch422g->base.read_output_reg = read_output_reg;
    ch422g->base.read_od_output_reg = read_od_output_reg;
...
static esp_err_t read_input_reg(esp_io_expander_handle_t handle, uint32_t *value)
{
	...
	ESP_RETURN_ON_ERROR(
        i2c_master_write_to_device(ch422g->i2c_num, ch422g->i2c_address, &temp, 1, pdMS_TO_TICKS(I2C_TIMEOUT_MS)),
...
static esp_err_t write_od_output_reg(esp_io_expander_handle_t handle, uint32_t value)
{
    ...
    uint8_t data = (uint8_t)value;
    ESP_RETURN_ON_ERROR(
        i2c_master_write_to_device(ch422g->i2c_num, CH422G_REG_OD_OUT, &data, 1, pdMS_TO_TICKS(I2C_TIMEOUT_MS)),
        TAG, "Write output reg failed");
    ch422g->regs.od_output = data;
    return ESP_OK;
}
