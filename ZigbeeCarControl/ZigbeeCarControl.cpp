...
ZigbeeCarControl::ZigbeeCarControl(uint8_t endpoint) : ZigbeeEP(endpoint) {
    _device_id = ESP_ZB_HA_ON_OFF_SWITCH_DEVICE_ID; // Phù hợp với vai trò gửi lệnh và nhận báo cáo
    _instance = this;
    ...
    // Client Cluster: để gửi lệnh đến End Device
    ...
    esp_zb_cluster_list_add_level_cluster(_cluster_list, esp_zb_zcl_attr_list_create(ESP_ZB_ZCL_CLUSTER_ID_LEVEL_CONTROL), ESP_ZB_ZCL_CLUSTER_CLIENT_ROLE);
    ...
    // Server Cluster: để nhận báo cáo trạng thái từ End Device
    ...
    esp_zb_cluster_list_add_fan_control_cluster(_cluster_list, esp_zb_fan_control_cluster_create(&fan_cfg), ESP_ZB_ZCL_CLUSTER_SERVER_ROLE);
    ...
    ESP_LOGI(TAG, "ZigbeeCarControl khoi tao thanh cong tren Endpoint %d, san sang gui lenh va nhan bao cao", endpoint);
}
...
void ZigbeeCarControl::findCarControlServer() {
    ...
    param.num_in_clusters = 3;                  // Chúng ta đang tìm 1 Cluster (Fan Control) mà thiết bị khác là Server
    param.cluster_list = cluster_list;
    param.num_out_clusters = 0;                 // Không quan tâm Out-Clusters của thiết bị đích trong tìm kiếm này

    ESP_LOGI(TAG, "Dang tim kiem ZigbeeCarDevice co On/Off, Level Control, hoac Fan Control Servers...");
    esp_err_t status = esp_zb_zdo_match_cluster(&param, ZigbeeCarControl::findCbWrapper, this);
    if (status != ESP_OK) {
        ESP_LOGE(TAG, "Gui yeu cau Match Descriptor that bai, trang thai: %d", status);
    }
}
